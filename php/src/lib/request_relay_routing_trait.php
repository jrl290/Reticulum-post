...
trait RequestRelayRoutingTrait
{
    ...
    private function relayPacketBase64(string $rawBase64, array $packet): string
    {
        ...
        $forwardedHops = $this->transportObservedHops($packet);
    ...
    private function peekReversePath(string $truncatedHashHex, string $outboundInterfaceId): ?array
    {
        $select = $this->db->prepare(
                        "SELECT rpe.truncated_hash_hex, rpe.received_interface_id, rpe.outbound_interface_id
                         FROM reverse_path_entries AS rpe
    ...
    private function relayAcceptedPacket(string $sourceInterfaceId, string $rawBase64, array $packet): int
    {
        ...
        // For validated announces relayed as periodic refresh, bump the
        // path entry timestamp so we don't re-relay on every subsequent
        // validated announce.
        if ($queued > 0 && (int) ($packet['packet_type'] ?? -1) === 1 && (string) ($packet['announce_status'] ?? '') === 'validated') {
            $this->touchPathEntryTimestamp((string) ($packet['destination_hash_hex'] ?? ''));
        }
    ...
    private function queueRelayPathRequestIfNeeded(string $sourceInterfaceId, array $packet): void
    {
        ...
        $targets = $this->allOtherInterfaceIds($sourceInterfaceId);
        if ($targets === []) {
            return;
        }
    ...
    private function rememberLinkTransportRelay(string $sourceInterfaceId, string $targetInterfaceId, string $rawBase64, array $packet): void
    {
        ...
        // remaining_hops MUST be the path distance from this relay to the
        // destination, NOT the observed hop count of the incoming packet.
        // Python: path_table[dest][IDX_PT_HOPS]  Rust: PathEntry.hops
        // Fall back to transportObservedHops only when no path exists.
        // See HOPS.md §5 Bug #6.
    ...
    private function linkTransportTargetInterfaceId(string $sourceInterfaceId, int $observedHops, array $linkEntry): ?string
    {
        ...
        if ($sourceInterfaceId === $outboundInterfaceId && $remOk) {
            return $receivedInterfaceId;
        }

        if ($sourceInterfaceId === $receivedInterfaceId && $tknOk) {
            return $outboundInterfaceId;
        }
    ...
    /**
     * Return the transport-observed hop count for a packet.
     *
     * The inbound handler (RequestInboundBatchTrait) increments hops on
     * receipt, matching Python Transport.inbound() and Rust
     * transport.rs:4438. This function returns the post-inbound value
     * directly — it no longer adds +1 as compensation.
     *
     * See HOPS.md §4.3, §5 Bug #3.
     */
    private function transportObservedHops(array $packet): int
    {
        return (int) ($packet['hops'] ?? 0);
    }
