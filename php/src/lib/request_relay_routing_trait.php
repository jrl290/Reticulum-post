...
trait RequestRelayRoutingTrait
{
    ...
    private function proofRelayPacketBase64(string $rawBase64, array $packet): string
    {
        ...
        // hop count set by the responder (the browser). The NAS Transport
        // must accept proofs arriving on the correct interface regardless
        // of hop count (the signature check provides security). See the
        // corresponding fix in RNS/Transport.py: the remaining_hops==0
        // fallback should be extended to all interface-matched proofs.
        return $rawBase64;
    }
    ...
    private function linkTransportTargetInterfaceId(string $sourceInterfaceId, int $observedHops, array $linkEntry): ?string
    {
        ...
        // Relaxed hop matching: PHP relay hops may differ by ±1 from
        // the stored values due to non-RNS relay hop increment behavior.
        $remOk = $remainingHops < 0 || abs($observedHops - $remainingHops) <= 1;
        $tknOk = $takenHops < 0 || abs($observedHops - $takenHops) <= 1;
