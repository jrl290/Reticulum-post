...
require_once __DIR__ . '/request_runtime.php';
require_once __DIR__ . '/database.php';
require_once __DIR__ . '/request_control_plane_trait.php';
...
require_once __DIR__ . '/lib/request_post_interface_wake_trait.php';
...
final class Config
{
    ...
    private static function normalizeConfig(string $projectRoot, array $config): array
    {
        ...
        return self::normalizePostInterfacePeers($config);
    }
    ...
    /**
     * Convert [[InterfaceName]] type=PostInterface entries in [interfaces]
     * into post_interface_peers entries.  PostInterface is the universal
     * poll+wake transport — same code for Python, JS, and PHP peers.
     */
    private static function normalizePostInterfacePeers(array $config): array
    {
        $interfaces = $config['interfaces'] ?? [];
        if (!is_array($interfaces)) {
            return $config;
        ...
        $peers = $config['post_interface_peers'] ?? [];
        if (!is_array($peers)) {
            $peers = [];
        }
        ...
        foreach ($interfaces as $interfaceName => $interfaceConfig) {
            ...
            $peerId = (string) $interfaceName;
            $nodeUrl = rtrim((string) ($interfaceConfig['node_url'] ?? ''), '/');
            $wakeUrl = isset($interfaceConfig['wake_url']) && is_string($interfaceConfig['wake_url'])
                ? rtrim(trim($interfaceConfig['wake_url']), '/')
                : null;
            ...
            $peers[$peerId] = [
                ...
                'node_url' => $nodeUrl,
                'wake_url' => $wakeUrl,
                'bitrate' => (int) ($interfaceConfig['bitrate'] ?? 62500),
                'mtu' => (int) ($interfaceConfig['mtu'] ?? 500),
                'http_timeout_seconds' => (int) ($interfaceConfig['http_timeout_seconds'] ?? 15),
                'connect_timeout_seconds' => (int) ($interfaceConfig['connect_timeout_seconds'] ?? 5),
            ];
        }
...
final class Storage
{
    ...
    use RequestPostInterfaceWakeTrait;
    ...
    public function handle(string $method, string $uri, array $server): never
    {
        try {
            ...
            if ($method === 'POST' && $path === '/v1/wake') {
                ...
                $this->runInterfaceRequestPrelude();
                $result = $this->storage->handleIncomingPostInterfaceWake($wakerUrl);
                $this->runInterfaceRequestEpilogue();
