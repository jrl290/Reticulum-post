...
trait RequestSchemaTrait
{
     ...
     *
     * MySQL rejects TEXT/BLOB columns in keys without a prefix length.
     * SQLite accepts TEXT anywhere.  This wrapper replaces TEXT with
     * VARCHAR(1024) for MySQL so all CREATE TABLE / INDEX statements work.
     */
    private function execDdl(string $sql): void
    {
        if ($this->backend === 'mysql') {
            // MySQL rejects TEXT/BLOB in keys.  All hex-hash / ID columns
            // are ≤ 64 chars, so VARCHAR(255) is safe for keyed columns.
            // Restore TEXT for payload / JSON columns that may exceed 255.
    ...
    /** Create an index, silently ignoring duplicate-index errors on either backend. */
    private function ensureIndex(string $sql): void
    {
        try {
            $this->execDdl($sql);
        } catch (\PDOException $e) {
            // MySQL 5.7 doesn't support IF NOT EXISTS for indexes
    ...
    public function migrate(): void
    {
        ...
        // ------------------------------------------------------------------
        // transport_state (key-value store)
        // ------------------------------------------------------------------
        $this->execDdl(
        ...
        $this->ensureIndex('CREATE INDEX idx_post_interface_peers_remote_node_url ON post_interface_peers(remote_node_url)');
    }
