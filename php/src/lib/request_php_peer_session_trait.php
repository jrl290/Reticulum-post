...
trait RequestPhpPeerSessionTrait
{
    public function ensurePhpPeerLocalSession(string $peerName, ?string $initialRemoteUrl = null): array
    {
        ...
        $insert->bindValue(':peer_name', $peerName, SQLITE3_TEXT);
        $insert->bindValue(':local_interface_id', $localInterfaceId, SQLITE3_TEXT);
        $insert->bindValue(':local_session_token', $localSessionToken, SQLITE3_TEXT);
        $insert->bindValue(':remote_url', $remoteUrl, $remoteUrl === null ? SQLITE3_NULL : SQLITE3_TEXT);
        $insert->bindValue(':updated_at', $now, SQLITE3_INTEGER);
        $insert->execute();
    ...
    public function phpPeerSessionByRemoteUrl(string $remoteUrl): ?array
    {
        ...
        $stmt->bindValue(':remote_url', trim($remoteUrl), SQLITE3_TEXT);
    ...
    public function phpPeerSessionByInterfaceId(string $interfaceId): ?array
    {
        ...
        $stmt->bindValue(':local_interface_id', $interfaceId, SQLITE3_TEXT);
        $row = $stmt->execute()->fetchArray(SQLITE3_ASSOC);
        if (!is_array($row)) {
            return null;
        }
    ...
    public function phpPeerRemoteUrl(string $peerName): ?string
    {
        ...
        $stmt->bindValue(':peer_name', $peerName, SQLITE3_TEXT);
        $row = $stmt->execute()->fetchArray(SQLITE3_ASSOC);
        if (!is_array($row)) {
            return null;
        }
